/****************************************************************************
 * This example of using OpenACC unstructured data regions was derived from
 * the Rodinia Hotspot benchmark.  Please see the enclosed LICENSE file.
 ...
 * by one time step
 */
void single_iteration(int row, int col,
...
#pragma acc declare deviceptr(result), present_or_copy(temp[0:row*col]), pcopyin(power[0:row*col])
#ifdef _OPENMP
 ...
 */
void compute_tran_temp(int num_iterations, int row, int col) 
{
	#ifdef VERBOSE
	int i = 0;
	#endif
     ...
     for (int i = 0; i < num_iterations ; i++)
	{
		...
		#endif
		single_iteration(row, col, Cap, Rx, Ry, Rz, step);
	}	
...
#pragma acc enter data copyin(temp[0:grid_rows*grid_cols],power[0:grid_rows*grid_cols])
}
...
int main(int argc, char **argv)
{
	...
	init_data(grid_rows, grid_cols);
	printf("Start computing the transient temperature\n");
	compute_tran_temp(sim_time, grid_rows, grid_cols);
...
#ifdef OUTPUT
#pragma acc update host(temp[0:size])
        for(i=0; i < grid_rows * grid_cols; i++)
        fprintf(stdout, "%d\t%g\n", i, temp[i]);
#endif

	/* cleanup	*/
#pragma acc exit data delete(temp[0:size],power[0:size])
	free(temp);
	free(power);
#ifdef _OPENACC
	acc_free(result);
